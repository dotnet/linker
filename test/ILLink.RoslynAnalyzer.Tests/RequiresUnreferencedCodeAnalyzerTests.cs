...
class C
{
	...
	static void M2 (Type t)
	{
		...
		[Fact]
		public Task TestRequiresInClassAccessedByStaticMethod ()
		{
			var source = @"
using System.Diagnostics.CodeAnalysis;
class C
{
	...
	static void TestRequiresInClassAccessedByStaticMethod ()
	{
		ClassWithRequires.StaticMethod ();
			...
			return VerifyRequiresUnreferencedCodeAnalyzer (source,
				// (16,3): warning IL2026: Using member 'C.ClassWithRequires.StaticMethod()' which has 'RequiresUnreferencedCodeAttribute' can break functionality when trimming application code. Message for --ClassWithRequires--.
				VerifyCS.Diagnostic (DiagnosticId.RequiresUnreferencedCode).WithSpan (15, 3, 15, 36).WithArguments ("C.ClassWithRequires.StaticMethod()", " Message for --ClassWithRequires--.", ""));
		}

		[Fact]
		public Task TestRequiresInClassAccessedByCctor ()
		{
			var source = @"
using System.Diagnostics.CodeAnalysis;
class C
{
	...
	static void TestRequiresInClassAccessedByCctor ()
	{
		var classObject = new ClassWithRequires ();
			...
			return VerifyRequiresUnreferencedCodeAnalyzer (source,
				// (16,3): warning IL2026: Using member 'C.ClassWithRequires.StaticMethod()' which has 'RequiresUnreferencedCodeAttribute' can break functionality when trimming application code. Message for --ClassWithRequires--.
				VerifyCS.Diagnostic (DiagnosticId.RequiresUnreferencedCode).WithSpan (16, 3, 16, 36).WithArguments ("C.ClassWithRequires.StaticMethod()", " Message for --ClassWithRequires--.", ""));
		}

		[Fact]
		public Task TestRequiresOnBaseButNotOnDerived ()
		{
			var source = @"
using System.Diagnostics.CodeAnalysis;
class C
