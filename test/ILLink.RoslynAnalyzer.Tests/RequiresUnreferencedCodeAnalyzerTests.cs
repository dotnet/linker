...
class C
{
    ...
    int M2() => M1();
			...
			return VerifyRequiresUnreferencedCodeAnalyzer (TestRequiresWithMessageOnlyOnMethod,
				// (8,17): warning IL2026: 'C.M1()' method has `RequiresUnreferencedCodeAttribute` which can break functionality when trimming application code. message.
...
class C
{
	...
	static void RequiresWithMessageAndUrl ()
	{
			...
			return VerifyRequiresUnreferencedCodeAnalyzer (MessageAndUrlOnMethod,
				// (8,3): warning IL2026: 'C.RequiresWithMessageAndUrl()' method has `RequiresUnreferencedCodeAttribute` which can break functionality when trimming application code. Message for --RequiresWithMessageAndUrl--. https://helpurl
	...
	static int PropertyRequires {
		...
		get { return 42; }
			...
			return VerifyRequiresUnreferencedCodeAnalyzer (PropertyRequires,
				// (8,7): warning IL2026: 'C.PropertyRequires.get' method has `RequiresUnreferencedCodeAttribute` which can break functionality when trimming application code. Message for --getter PropertyRequires--. 
...
class C
{
	...
	static int PropertyRequires {
		...
		set { }
			...
			return VerifyRequiresUnreferencedCodeAnalyzer (PropertyRequires,
				// (8,3): warning IL2026: 'C.PropertyRequires.set' method has `RequiresUnreferencedCodeAttribute` which can break functionality when trimming application code. Message for --setter PropertyRequires--.
