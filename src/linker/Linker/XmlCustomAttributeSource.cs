					...
					foreach (ParameterDefinition parameter in method.Parameters) {
						if (paramName == parameter.Name) {
							if (_attributes.ContainsKey (parameter))
								_context.LogWarning ($"There are duplicate parameter names for '{paramName}' inside '{method.GetDisplayName ()}' in '{_xmlDocumentLocation}'", 2024, _xmlDocumentLocation);
		...
		void ProcessReturnParameters (MethodDefinition method, XPathNodeIterator iterator)
		{
			...
			while (iterator.MoveNext ()) {
				if (firstAppearance) {
					...
					_context.LogWarning ($"There is more than one return parameter specified for '{method.GetDisplayName ()}' in '{_xmlDocumentLocation}'", 2023, _xmlDocumentLocation);
				}
			}
		}
