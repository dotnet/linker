	...
	public class RemoveUnreachableBlocksStep : BaseStep
	{
		...
		class BodyReducer
		{
			...
			public bool RewriteBody ()
			{
				...
				var intents = RemoveConditions ();
				if (intents.Count == 0)
					return false;

				var reachableInstrs = GetReachableInstructionsMap (intents, out var unreachableEH);
				...
				if (reachableInstrs == null)
					return false;
				reachableInstrs = GetReachableInstructionsMap (intents, out unreachableEH);
				if (reachableInstrs == null)
					return false;
			...
			Dictionary<(int, int), Action> RemoveConditions ()
			{
				var intents = new Dictionary<(int, int), Action> ();
				object left, right;
				...
				for (int i = 0; i < FoldedInstructions.Count; ++i) {
					...
					if (opcode.FlowControl == FlowControl.Cond_Branch) {
						if (opcode.StackBehaviourPop == StackBehaviour.Pop1_pop1) {
							...
							if (left is int lint && right is int rint) {
								int capturei = i;
								intents [(capturei - 2, capturei)] = delegate {
									RewriteToNop (capturei - 2);
									RewriteToNop (capturei - 1);

									if (IsComparisonAlwaysTrue (opcode, lint, rint)) {
										Rewrite (capturei, Instruction.Create (OpCodes.Br, (Instruction)instr.Operand));
									} else {
										RewriteConditionToNop (capturei);
									}
								};
								continue;
							}
						...
						if (opcode.StackBehaviourPop == StackBehaviour.Popi) {
							if (i > 0 && GetConstantValue (FoldedInstructions [i - 1], out var operand)) {
								if (operand is int opint) {
									var capturei = i;
									intents [(capturei - 1, capturei)] = delegate {
										RewriteToNop (capturei - 1);
							...
							if (i > 3 && GetConstantValue (FoldedInstructions [i - 3], out operand) && operand is int opint2 && IsPairedStlocLdloc (FoldedInstructions [i - 2], FoldedInstructions [i - 1])) {
								var capturei = i;
								intents [(capturei - 3, capturei)] = delegate {
									RewriteToNop (capturei - 3);
									RewriteToNop (capturei - 2);
									RewriteToNop (capturei - 1);
						...
						if (left is int lint && right is int rint) {
							...
							intents [(capturei - 2, capturei)] = delegate {
								...
								if (IsComparisonAlwaysTrue (instr.OpCode, lint, rint)) {
									Rewrite (capturei, Instruction.Create (OpCodes.Ldc_I4_1));
								} else {
									Rewrite (capturei, Instruction.Create (OpCodes.Ldc_I4_0));
								}
							};
						}
					...
					case Code.Cgt_Un:
						...
						if (left == null && right == null) {
							int capturei = i;
							intents [(capturei, capturei)] = delegate {
								Rewrite (capturei, Instruction.Create (OpCodes.Ldc_I4_0));
							};
						}
						break;
			...
			BitArray GetReachableInstructionsMap (Dictionary<(int, int), Action> intents, out List<ExceptionHandler> unreachableHandlers)
			{
			...
			static void RemoveIfReachable (Dictionary<(int, int), Action> intents, int index)
			{
				foreach (var intent in intents) {
					if (intent.Key.Item1 <= index && intent.Key.Item2 >= index) {
