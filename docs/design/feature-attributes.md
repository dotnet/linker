...
If we are willing to revisit the design of feature switches, we might consider other ways to represent the features in attributes.

### Typed feature definitions

Instead of using strings to represent features, we could use attributes. There could be an abstract base `FeatureDefinition` attribute, whose subclasses represent specific features. For example, the above could be rewritten as:
...
In this case, the name of the feature passed to the linker would need to be determined. We could make it the fully-qualified attribute name (minus "Attribute"), for example `--feature AwesomeFeature false`.
...
// TODO: find an actual example
```csharp
class AwesomeClass
{
    [Nullable] // emitted by the compiler for the return type
    public bool? NullableMethod () {}
}
```

If linking with `--feature RuntimeNullableAttributes false`, this would remove the `Nullable` instance from `NullableMethod`.
...
// TODO: Link to Michal's old proposal, Immo's proposal